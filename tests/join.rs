...
fn test_join_different_types() {
    ...
    let result = rt.block_on(async {
        let (num, text, vec) = join!(
            let num = async { 100i32 },
            let text = async { String::from("test") },
            let vec = async { vec![1, 2, 3] }
        );
        (num, text, vec)
    });
...
fn test_join_with_captured_values() {
    ...
    let result = rt.block_on(async move {
        let (a, b) = join!(
            let a = async move { value * multiplier },
            let b = async move { value + multiplier }
        );
        (a, b)
    });
...
fn test_join_with_result_types() {
    ...
    let result = rt.block_on(async {
        let (ok_result, err_result) = join!(
            let ok_result = async { Ok::<i32, &str>(100) },
            let err_result = async { Err::<i32, &str>("error") }
        );
